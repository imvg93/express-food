...
export default function TransportPage() {
  ...
  const items = expenses.filter((e) => e.category === "transport");
  const total = items.reduce((s, e) => s + e.amount, 0);
  return (
    <div className="h-full flex flex-col">
      <ExpenseTable expenses={items} onRowClick={setSelected} onAdd={() => setAddOpen(true)}
        title="Transport & Logistics Expenses" totalThisMonth={total} />
      <ExpenseDetailDrawer expense={selected} open={!!selected} onClose={() => setSelected(null)} />
      <AddExpenseModal open={addOpen} onClose={() => setAddOpen(false)} category="transport" categoryLabel="Transport & Logistics" />
    </div>
  );
}
