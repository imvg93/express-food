...
export default function UtilitiesPage() {
  ...
  const items = expenses.filter((e) => e.category === "utilities");
  const total = items.reduce((s, e) => s + e.amount, 0);
  return (
    <div className="h-full flex flex-col">
      <ExpenseTable expenses={items} onRowClick={setSelected} onAdd={() => setAddOpen(true)}
        title="Utility Expenses" totalThisMonth={total} />
      <ExpenseDetailDrawer expense={selected} open={!!selected} onClose={() => setSelected(null)} />
      <AddExpenseModal open={addOpen} onClose={() => setAddOpen(false)} category="utilities" categoryLabel="Utilities" />
    </div>
  );
}
