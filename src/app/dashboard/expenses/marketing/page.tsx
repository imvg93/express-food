...
export default function MarketingPage() {
  ...
  const items = expenses.filter((e) => e.category === "marketing");
  const total = items.reduce((s, e) => s + e.amount, 0);
  return (
    <div className="h-full flex flex-col">
      <ExpenseTable expenses={items} onRowClick={setSelected} onAdd={() => setAddOpen(true)}
        title="Marketing & Promotion Expenses" totalThisMonth={total} />
      <ExpenseDetailDrawer expense={selected} open={!!selected} onClose={() => setSelected(null)} />
      <AddExpenseModal open={addOpen} onClose={() => setAddOpen(false)} category="marketing" categoryLabel="Marketing & Promotion" />
    </div>
  );
}
